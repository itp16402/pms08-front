...
@Component({
  selector: 'app-planning-first-step',
  templateUrl: './planning-first-step.component.html',
  styleUrls: ['./planning-first-step.component.scss']
})
export class PlanningFirstStepComponent implements OnInit, AfterViewInit {
  ...
  setTableNameToLocalStorage(tableName: any): any {
    if (tableName.tableName === 'a221') {
      this.router.navigate([this.currentLang, 'home', this.orderId, 'statutory', 'planning', 'planning-first-step', '1', tableName.tableName]);
    }
  }
    ...
    if (event.which === 2) {
      if (tableName.tableName === 'a221') {
        this.router.navigate([this.currentLang, 'home', this.orderId, 'statutory', 'planning',
          'planning-first-step', '1', tableName.tableName]);
      }
    }
  }
}
